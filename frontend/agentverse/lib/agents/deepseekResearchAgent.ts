...
function buildSystemPrompt(): string {
  return [
    ...
    'Return this exact JSON shape: {"summary":"one sentence professional achievement summary","output":"complete Markdown research artifact","artifacts":["Project Research Brief","Risk Assessment Notes","Onchain Research Summary"],"scoreSuggestion":1}',
...
function buildFallbackResult(agent: AgentProfile, task: AgentTask): AgentTaskResult {
  return {
    taskId: task.id,
    agentId: agent.id,
    summary: "DeepSeek unavailable, generated fallback demo artifact.",
    output: [
      "# Fallback Demo Artifact",
      "",
      "## Task Overview",
      `${agent.name} was assigned to produce a portfolio-ready research artifact for: ${task.title}.`,
      "DeepSeek was unavailable or returned an invalid response, so a fallback demo artifact was generated instead.",
      "",
      "## Key Findings",
    ...
    artifacts: [
      `${task.input} Research Brief`,
      `${task.input} Risk Assessment Notes`,
      `${task.input} Onchain Research Summary`,
    ],
    scoreSuggestion: 3,
    status: "success",
